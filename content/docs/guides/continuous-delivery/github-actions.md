...
{{% notes type="info" %}}
This guide is reference material for Pulumi's GitHub Action v2. If you are using v1 of the Pulumi GitHub Action, then
please note it has been deprecated and will be EOLed August 31st 2021.
{{% /notes %}}
...
To allow your GitHub Action to leave Pull Request comments, you'll need to set
`comment-on-pr` and `github-token` to the list of inputs
passed to the action. Update the action to be as follows:
...
* The action now runs natively, so the action workflow needs to have the correct environment configured. There
  are sample workflows available in out [Actions repository](https://github.com/pulumi/actions/tree/master/.github/workflows).
  For examples, if you are running a NodeJS (for example) app then you need to ensure that your action has NodeJS available to it:
...
* The action will no longer run `npm ci | npm install | pip3 install | pipenv install`. Please ensure that you are installing
  your dependencies before Pulumi commands are executed, e.g.:
